...
typedef struct {
    ...
    // gridHistory is a dynamically allocated array of grids
    int ***gridHistory;
    int gridHistoryCount;
    int gridHistoryCapacity;
    int gCurr;
    int heuristicScore;
    double logMax;
    double logSecondMax;
...
void initializeGameState(GameState *state, int initialGrid[GRID_SIZE][GRID_SIZE]) {
    ...
    // Initialize gridHistory
    state->gridHistoryCapacity = 10;
    state->gridHistoryCount = 0;
    state->gridHistory = (int ***)malloc(state->gridHistoryCapacity * sizeof(int **));
    // Incrementally add grids to history as needed
...
// Function to add a grid to gridHistory
void addToGridHistory(GameState *state, int grid[GRID_SIZE][GRID_SIZE]) {
    if(state->gridHistoryCount >= state->gridHistoryCapacity) {
        state->gridHistoryCapacity *= 2;
        state->gridHistory = (int ***)realloc(state->gridHistory, state->gridHistoryCapacity * sizeof(int **));
    }
    // Allocate memory for the new grid
    state->gridHistory[state->gridHistoryCount] = (int **)malloc(GRID_SIZE * sizeof(int *));
    for(int i = 0; i < GRID_SIZE; i++) {
        state->gridHistory[state->gridHistoryCount][i] = (int *)malloc(GRID_SIZE * sizeof(int));
        for(int j = 0; j < GRID_SIZE; j++) {
            state->gridHistory[state->gridHistoryCount][i][j] = grid[i][j];
        }
    }
    state->gridHistoryCount++;
}

// Function to free gridHistory memory
void freeGridHistory(GameState *state) {
    for(int i = 0; i < state->gridHistoryCount; i++) {
        for(int j = 0; j < GRID_SIZE; j++) {
            free(state->gridHistory[i][j]);
        }
        free(state->gridHistory[i]);
    }
    free(state->gridHistory);
}
...
// Returns the number of next states generated
int generateNextStates(GameState currentState, GameState *nextStates) {
    ...
    for(int move = 0; move < 4; move++) {
        ...
        if(changed) {
            ...
            // Copy gridHistory from currentState
            newState.gridHistoryCount = currentState.gridHistoryCount;
            newState.gridHistoryCapacity = currentState.gridHistoryCapacity;
            newState.gridHistory = (int ***)malloc(newState.gridHistoryCapacity * sizeof(int **));
            for(int hIdx = 0; hIdx < currentState.gridHistoryCount; hIdx++) {
                newState.gridHistory[hIdx] = (int **)malloc(GRID_SIZE * sizeof(int *));
                for(int i = 0; i < GRID_SIZE; i++) {
                    newState.gridHistory[hIdx][i] = (int *)malloc(GRID_SIZE * sizeof(int));
                    for(int j = 0; j < GRID_SIZE; j++) {
                        newState.gridHistory[hIdx][i][j] = currentState.gridHistory[hIdx][i][j];
                    }
                }
            }
            // Add current grid to history
            addToGridHistory(&newState, currentState.grid);
    ...
    // Note: In this implementation, nextStates is assumed to have enough space
    // The function returns the number of next states generated, which needs to be managed
    // For simplicity, returning 0 as a placeholder
    return 0;
}
...
Tuple aStarAlgorithm(GameState initialState) {
    ...
    int cntSize = 0;
    int cntCapacity = 10;
    cnt = (int *)malloc(cntCapacity * sizeof(int));

    int *values = NULL;
    int valuesSize = 0;
    int valuesCapacity = 10;
    values = (int *)malloc(valuesCapacity * sizeof(int));
    ...
    while(openedCount > 0) {
        ...
        for(int i = 0; i < GRID_SIZE && !goalFound; i++) {
            for(int j = 0; j < GRID_SIZE && !goalFound; j++) {
                if(currentState.grid[i][j] == 2048) {
                    // Add current grid to history
                    addToGridHistory(&currentState, currentState.grid);
                    // Set return values
                    result.gameState = currentState;
                    result.iterations = count;
                    result.cnt = cnt;
                    result.cntSize = cntSize;
        ...
        // Track iteration count and max values
        if(cntSize >= cntCapacity) {
            cntCapacity *= 2;
            cnt = (int *)realloc(cnt, cntCapacity * sizeof(int));
        }
        cnt[cntSize++] = count;
        ...
        for(int i = 0; i < GRID_SIZE; i++) {
            for(int j = 0; j < GRID_SIZE; j++) {
                if(currentState.grid[i][j] > maxTile)
                    maxTile = currentState.grid[i][j];
            }
        }

        if(valuesSize >= valuesCapacity) {
            valuesCapacity *= 2;
            values = (int *)realloc(values, valuesCapacity * sizeof(int));
        }
        values[valuesSize++] = maxTile;
...
int main() {
    ...
    // Print max tile values
    /*
    printf("Max tile values: ");
    for(int i = 0; i < result.valuesSize; i++) {
    ...
    */
    ...
    freeGridHistory(&result.gameState);
    free(result.cnt);
    free(result.values);
